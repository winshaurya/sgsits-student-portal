...
echo '<div class="navbar navbar-inverse bg-white">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="javascript:void(0)" onclick="check_content(\'Ajax.php?modname=miscellaneous/Portal.php\');" onmousedown="document.getElementById(\'header\').innerHTML = \'Home\'; document.getElementById(\'cframe\').src = \'Bottom.php?modcat=home\'"><img src="https://www.sgsitsindore.in/img/sgsits_logo.png" alt="SGSITS"></a>
    ...
    if ($allowed || $_SESSION['take_mssn_attn']) {
        ...
        if (User('USERNAME')) {
            ...
            if (false && function_exists('query')) {
                ...
                // Hard-coded external logging connection removed for security.
                // If you want to log hacking attempts to the local DB, use the app DB via db_start() and DBQuery():
                // $conn = db_start();
                // $sql = "INSERT INTO hacking_log (HOST_NAME,IP_ADDRESS,LOGIN_DATE,VERSION,PHP_SELF,DOCUMENT_ROOT,SCRIPT_NAME,MODNAME,USERNAME) values('$_SERVER[SERVER_NAME]','$ip','" . date('Y-m-d') . "','$openSISVersion','$_SERVER[PHP_SELF]','$_SERVER[DOCUMENT_ROOT]','$_SERVER[SCRIPT_NAME]','" . optional_param('modname', '', PARAM_CLEAN) . "','" . User('USERNAME') . "')";
                // $conn->query($sql);
            }
        }
        exit;
